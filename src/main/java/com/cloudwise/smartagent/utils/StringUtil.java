...
public abstract class StringUtil {
	...
	public static String list2Str(Collection list, String connector) {
	...
	public static String MD5(String string) {
		...
		try {
			...
			logger.error("Error happens in " + e);
		}
		return s;
	}
	...
	/**
	 * @param source
	 *            The source string to perform replacements on.
	 * @param find
	 *            The substring to find in source.
	 * @param replace
	 *            The string to replace 'find' within source
	 * @return The source string, with all occurrences of 'find' replaced with
	 *         'replace'
	 */
	public static String replace(String source, String find, String replace) {
		...
		for (fromIndex = 0; (idx = source.indexOf(find, fromIndex)) != -1; fromIndex = idx
				+ findLen) {
	...
	public static String remove(String source, String find) {
		...
		try {
			if (source != null && sourceLen > 0 && find != null && findLen > 0) {
				...
				for (fromIndex = 0, idx = 0; (fromIndex = source.indexOf(find,
						idx)) != -1; idx = fromIndex + findLen) {
	 ...
	 * 
	 * @param objs
	 *            List of objects to implode (elements may not be null)
	 * @param delim
	...
	/**
	 * Split a string on delimiter boundaries, and place each element into an
	 * array.
	 * 
	 * @param s
	 *            String to split up
	 ...
	 * subcomponents. If there is an uneven number of quotes, a parse error will
	 * be thrown.
	 * 
	 * @param arg
	 *            String to parse
	 ...
	 *             indicating there was a quoting error
	 */
	 ...
	 *         the prefix removed, and the resultant string trimmed. If value
	 *         does not start with prefix, value is returned as-is.
	 */
	...
	public static String pluralize(String word) {
		...
		if (word.endsWith("y")) {
			...
			if (next2last != 'a' && next2last != 'e' && next2last != 'i'
					&& next2last != 'o' && next2last != 'u' && next2last != 'y') {
	...
	public static String getStackTrace(Throwable t) {
		...
		try {
			...
			return "\n\nStringUtil.getStackTrace " + "GENERATED EXCEPTION: '"
					+ e.toString() + "' \n\n";
		}
	}
	 ...
	 * @return A new string that is <code>str</code> capitalized. Returns
	 *         <code>null</code> if str is null.
	 */
	...
	/**
	 * Return a variant of 'str' which contains the beginning and end of the
	 * string, but places '...' in the middle to limit the maximum length of the
	 * string.
	 * 
	 * @param str
	 *            String to shorten
	...
	/**
	 * Escapes a minimal set of metacharacters with their regular expression
	 * escape codes.
	 */
